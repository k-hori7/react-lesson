...
const arg4: number[] = [1, 2, 3, 2, 4, 5, 6, 5, 6];
console.log(removeDuplication(arg4));

// 問題5: テンプレートリテラルを使用して、引数で受け取った名前を元に「こんにちは、○○さん」と出力する関数を作成して実行してください。
// 例) 引数: '太郎', 出力: こんにちは、太郎さん
const hello = (name: string): void => console.log(`こんにちは、${name}さん`);
const arg5: string = "太郎";
hello(arg5);

// 問題6: 配列を引数として受け取り、各要素を2倍にした新しい配列を返す関数を作成して実行してください。
// 例) 引数: [1, 2, 3], 返り値: [2, 4, 6]
const doubleArray = (array: number[]): number[] => array.map((num) => num * 2);
const arg6: number[] = [1, 2, 3];
...
const stringArrayToIndexedObject = (array: string[]): IndexedValue[] => {
...
const findByAge = (array: NameAgeObj[], age: number): NameAgeObj[] =>
  array.filter((obj) => obj.age === age);
const arg8: NameAgeObj[] = [
  { name: "太郎", age: 20 },
  { name: "次郎", age: 30 },
  { name: "三郎", age: 40 },
];
...
const getDoubledEven = (array: number[]): number[] =>
  array.filter((num) => num % 2 === 0).map((num) => num * 2);
const arg9: number[] = [1, 2, 3, 4, 5, 6];
...
const add10FilterEvenSort = (array: number[]): number[] =>
  array
    .map((num) => num + 10)
    .filter((num) => num % 2 === 0)
    .sort((a, b) => a - b);
const arg10: number[] = [1, 2, 3, 4, 5, 6];
